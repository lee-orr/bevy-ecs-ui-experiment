...
use crate::{
    reactive_expression_handlers::{
        component_expression_change_handler, ComponentExpressionHandler, GetExpressionHandlers,
        ReactiveExpressionHandler,
    },
    ui_asset::UiNodeTree,
    ui_plugin::{spawn_ui, UiHandle},
    ArrayExpression, UIState, UiNode,
};
...
impl<T: UIState> Plugin for LogicalNodesPlugin<T> {
    fn build(&self, app: &mut App) {
        app.add_system(ui_if_else_changed::<T>.in_base_set(CoreSet::PostUpdate))
            .add_system(
                component_expression_change_handler::<T, UiIfElse, UiIfElseExpressionHandler>,
            );
    }
}
